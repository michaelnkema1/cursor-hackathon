...
def _row_to_public_map_safe(r: dict) -> IssuePublic:
    return IssuePublic(
        id=r["id"],
        reporter_id=None,
        status=r["status"],
        ...
        ai_severity=r.get("ai_severity") or r.get("severity"),
        ai_summary=None,
...
def _validate_reporter_media_path(path: str | None, reporter_id: str) -> str | None:
    if path is None:
        return None
    ...
    if not normalized.startswith(f"{reporter_id}/"):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Media path must be under your upload prefix",
        )
    return normalized


def _ensure_user_can_read_issue(supabase: Client, user: dict, issue_row: dict) -> dict:
    ...
    if role == "authority":
        ...
        if organization_id and str(issue_row.get("routed_organization_id")) == str(organization_id):
            return profile
...
def _can_view_duplicate_suggestions(profile: dict) -> bool:
    return (profile.get("role") or "citizen") in ("authority", "admin")
...
def submit_report(
    ...
    photo_path = _validate_reporter_media_path(body.photo_path, reporter_id)
    audio_path = _validate_reporter_media_path(body.audio_path, reporter_id)
    video_path = _validate_reporter_media_path(body.video_path, reporter_id)
...
def get_issue(
    ...
    profile = _ensure_user_can_read_issue(supabase, user, row)
    ...
    duplicates = (
        ...
        if _can_view_duplicate_suggestions(profile)
        else []
    )
...
def get_issue_media(
    ...
    if not row:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Issue not found")
    _ensure_user_can_read_issue(supabase, user, row)
