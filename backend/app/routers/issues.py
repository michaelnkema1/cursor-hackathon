...
def _normalize_owned_media_path(path: str | None, reporter_id: str) -> str | None:
    if path is None:
        return None
    ...
    if (
        ...
        raise HTTPException(
            ...
            detail="Media paths must reference files uploaded by the current user",
        )
    return normalized


def _ensure_user_can_view_issue(user: dict, supabase: Client, issue_row: dict) -> None:
    ...
    if role == "authority":
        ...
        if organization_id and str(organization_id) == str(routed_organization_id):
            return
...
def submit_report(
    ...
    photo_path = _normalize_owned_media_path(body.photo_path, reporter_id)
    audio_path = _normalize_owned_media_path(body.audio_path, reporter_id)
    video_path = _normalize_owned_media_path(body.video_path, reporter_id)
...
def get_issue(
    ...
    _ensure_user_can_view_issue(user, supabase, row)
